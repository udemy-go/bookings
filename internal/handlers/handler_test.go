...
func TestRepository_Reservation(t *testing.T) {
	...
	rr := httptest.NewRecorder() // it fakes the entire process from getting request to writing response
	...
	// test case whether the room not exist  
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
...
func TestRepository_PostReservation(t *testing.T) {
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=7834123098")
	...
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded") // set form header 
